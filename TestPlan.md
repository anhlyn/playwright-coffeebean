...
| TC-06 | Order lookup API returns a not-found response for an invalid order ID | Negative | 1. Send a POST request to https://api.valentinos-magic-beans.click/orders/lookup with a payload such as {"orderId": "2B42062A1", "email": "test@abc.com"}. 2. Review the response status and body. | The API returns a JSON payload similar to {"success": false, "data": {"message": "Order with ID 2B42062A1 not found for this email."}}. |
| TC-07 | Product detail API returns a not-found response for an invalid product ID | Negative | 1. Send a GET request to https://api.valentinos-magic-beans.click/products/1000. 2. Review the response status and body. | The API returns HTTP 404 with a JSON payload similar to {"success": false, "source": "dynamodb", "data": {"message": "Product with ID 1000 not found."}}. |
